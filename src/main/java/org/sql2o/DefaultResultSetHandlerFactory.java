...
public class DefaultResultSetHandlerFactory<T> implements ResultSetHandlerFactory<T> {
	...
	private static Getter getGetter(final Quirks quirks, final String propertyPath, final PojoMetadata metadata) {
				...
				public Object getProperty(Object obj) {
					try {
						...
						throw new Sql2oException(
								"Error trying to convert column " + propertyPath + " to type " + getter.getType(), e);
					}
				}
		...
		// dot path - long way
		// i'm too lazy now to rewrite this case so I just call old unoptimized
		// code...
			...
			return new Setter() {
				public void setProperty(Object obj, Object value) {
					try {
						...
						throw new Sql2oException(
								"Error trying to convert column " + propertyPath + " to type " + setter.getType(), e);
					}
				}
	...
	private static class Key {
		...
		public boolean equals(Object o) {
			...
			return f.metadata.equals(key.getMetadata()) && f.quirks == key.getQuirksMode()
	...
	private ResultSetHandler<T> newResultSetHandler0(final ResultSetMetaData meta) throws SQLException {
		...
		for (int i = 1; i <= columnCount; i++) {
			...
			// If more than 1 column is fetched (we cannot fall back to
			// executeScalar),
		...
		/**
		 * Fallback to executeScalar if converter exists, we're selecting 1
		 * column, and no property setter exists for the column.
		 */
			...
			public T handle(ResultSet resultSet) throws SQLException {
				if (useExecuteScalar) {
					try {
						...
						throw new Sql2oException(
								"Error occurred while converting value from database to type " + metadata.getType(), e);
					}
				}
